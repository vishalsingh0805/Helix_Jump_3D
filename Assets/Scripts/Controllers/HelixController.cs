...
public class HelixController : MonoBehaviour
{
    ...
    void StackCleared(int index)
    {
        ...
        if (clearedStack)
        {
            Debug.Log("Stack cleared - Starting animation");
            
            // Use animation instead of immediate destruction
            Stack stackComponent = clearedStack.GetComponent<Stack>();
            if (stackComponent != null)
            {
                stackComponent.BreakStack(); // This handles the animation and destruction
            }
            else
            {
                // Fallback to immediate destruction if no Stack component
                Destroy(clearedStack);
            }
        }
